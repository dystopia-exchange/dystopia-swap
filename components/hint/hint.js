...
function Hint(props) {
  ...
  const {hintText, open, anchor, handleClick, handleClose, horizontal, vertical = -90, showIcon = true, children, autoWidth} = props;
  ...
  return (
    <div
      ...
      <Popover
        classes={{
          paper: [classes.popoverPaper, appTheme === "dark" ? classes['popoverPaper--dark'] : classes['popoverPaper--light'], autoWidth ? classes.popoverPaperAuto : ''].join(' '),
        }}
        sx={{
          pointerEvents: 'none',
        }}
        ...
        anchorOrigin={{
          vertical,
          horizontal: horizontal || horizontal === 0 ? horizontal : windowWidth > 530 ? -200 : -257,
        }}>
        <div
          style={{
            position: 'relative',
